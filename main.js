...
function displayComputerChoice(computerSelection) {
  ...
  cpuImg.setAttribute('src', `assets/${computerSelection}.png`)
...
function checkForGameEnd(resultPanel) {
  if (playerWins >= 3 && playerWins >= computerWins) {
    resultPanel.textContent = `Congratulations! You win with a score of 
    ${playerWins}-${computerWins}.    
    Click here to play again!`;
  } else if (computerWins >= 3) {
    resultPanel.textContent = `Oh no! You lost with a score of
    ${playerWins}-${computerWins}.    
    Click here to play again!`;
  }
  if (playerWins === 3 || computerWins === 3) {
    ...
    resultPanel.addEventListener('click', resetGame)  
  }
}
...
//   computerWins = 0;
//   updateScore();
//   displayComputerChoice("robotFace");
//   listOfSelection.forEach((selected) => selected.addEventListener('click', handleChoice));
